...
export default function CareCards() {
    ...
    useEffect(() => {
        const fetchCarecards = async () => {
            try {
                ...
                const data: CareCard[] = await response.json();
                setCarecards(data);
            } catch (error) {
                console.error('Error fetching care cards:', error);
            }
        };
    ...
    const handleCardClick = (id: string) => {
        console.log(`Care card clicked with ID: ${id}`);
    };
            ...
            {carecards.map(carecard => (
                ...
                >
                    ...
                    <div className="text-start relative z-10">
                        <h2 className="text-[0.97rem] text-customText font-semibold">{carecard.title}</h2>
                        <p className="text-customText text-[0.95rem] italic">{carecard.description}</p>
                        <div className="flex items-center gap-2">
                            <p className="text-customText line-through">€{carecard.price.toFixed(2)}</p>
                            <p className="text-customPLNTS">€{carecard.price2?.toFixed(2)}</p>
                        </div>
